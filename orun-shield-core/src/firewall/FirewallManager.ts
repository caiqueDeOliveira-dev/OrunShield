 ...
 */
export class FirewallManager extends TypedEmitter<ShieldEventMap> {
  private readonly os: "windows" | "linux" | "macos" | "unsupported";
  ...
  private detectOS(): "windows" | "linux" | "macos" | "unsupported" {
    const p = platform();
  ...
  async addRule(rule: FirewallRule): Promise<void> {
    switch (this.os) {
      ...
      case "macos":
        throw new Error(
          "macOS usa PF (Packet Filter) via /etc/pf.conf — requer edição de arquivo de config, não é exposto via CLI simples. Implementação futura."
        );
      default:
  ...
  async blockIP(ip: string, ruleName = `orun-shield-block-${ip}`): Promise<void> {
    await this.addRule({
      name: ruleName,
      direction: "out",
      protocol: "any",
  ...
  private async addRuleWindows(rule: FirewallRule): Promise<void> {
    ...
    if (rule.remoteAddress) args.push(`remoteip=${rule.remoteAddress}`);
    if (rule.localPort) args.push(`localport=${rule.localPort}`);

    await this.run("netsh", args);
  }

  private async addRuleLinux(rule: FirewallRule): Promise<void> {
    const chain = rule.direction === "in" ? "INPUT" : "OUTPUT";
    const target = rule.action === "block" ? "DROP" : "ACCEPT";
    const args = ["-A", chain];

    if (rule.protocol !== "any") args.push("-p", rule.protocol);
    if (rule.remoteAddress) args.push(rule.direction === "in" ? "-s" : "-d", rule.remoteAddress);
    if (rule.localPort) args.push("--dport", String(rule.localPort));
   ...
   */
  private async removeRuleLinuxByComment(ruleName: string): Promise<void> {
  ...
  private findRuleLinesByChain(iptablesOutput: string, ruleName: string): { chain: string; lineNumber: number }[] {
    const results: { chain: string; lineNumber: number }[] = [];
    ...
    for (const line of iptablesOutput.split("\n")) {
      ...
      if (!currentChain || !line.includes(ruleName)) continue;
  ...
  private run(bin: string, args: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      child.on("close", (code) => {
        ...
        else reject(new Error(`${bin} finalizou com código ${code}: ${stderr}. Requer privilégios de administrador/root.`));
      });
    });
  }
}
